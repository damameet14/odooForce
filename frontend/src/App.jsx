...
function Login() {
  ...
  return <div className="login"><section><div className="login-brand"><span>VB</span><strong>VendorBridge</strong></div><div className="login-copy"><small>PROCUREMENT CONTROL CENTER</small><h1>One clear path from request to delivery.</h1><p>Manage vendor sourcing, approvals, purchase orders, invoices, and fulfillment in a single operational workspace.</p><div className="flow"><span>RFQ</span><i /><span>Quote</span><i /><span>Approve</span><i /><span>Deliver</span></div></div><footer>Secure role-based procurement operations</footer></section><form onSubmit={submit}><div><h2>Sign in</h2><p>Enter your credentials to access VendorBridge.</p><ErrorBox error={error} /><label>Email<input name="email" type="email" defaultValue="admin@vendorbridge.com" required /></label><label>Password<input name="password" type="password" defaultValue="password123" required /></label><Button type="submit" disabled={loading}>{loading ? "Signing in..." : "Sign in"}</Button><div className="demo"><strong>Getting started</strong><span>Login as Admin to create users, vendors, products, and categories</span><small>Default: admin@vendorbridge.com / password123</small></div></div></form></div>;
}
